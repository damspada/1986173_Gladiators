      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <aside className="space-y-4">
          <section className="tactical-panel p-3">
            <h3 className="text-[11px] uppercase tracking-[0.2em] text-zinc-300">Infrastructure</h3>
            <div className="mt-2 space-y-1 text-[11px] uppercase tracking-[0.1em] text-zinc-300">
              <p>Health: <span className="text-zinc-100">{infrastructureHealthLabel}</span></p>
              <p>Gateway: <span className="text-zinc-100">{infrastructure?.gateway ?? 'unknown'}</span></p>
              <p>Replicas: <span className="text-zinc-100">{healthyReplicas}/{totalReplicas}</span></p>
              <p>Active replica: <span className="text-zinc-100">{infrastructure?.activeReplica ?? 'n/a'}</span></p>
              <p>Last failover:</p>
          ...
          <section className="tactical-panel p-3">
            <div className="mb-2 flex items-center justify-between text-[11px] uppercase tracking-[0.16em] text-zinc-400">
              ...
              <span>{incidentClusters.length}</span>
            </div>
            {latestIncident ? (
              <div className="mb-2 rounded-sm border border-zinc-700/80 bg-zinc-900/60 px-2 py-2 text-xs">
                <p className="uppercase tracking-[0.08em] text-zinc-200">
                  Latest:{' '}
                  <ZoneNavLink zone={latestIncident.region} className="px-0 py-0 text-zinc-200 hover:text-cyan-200" />
                </p>
                <p className="mt-1 text-zinc-400">{latestIncident.count} events | {classificationLabel[latestIncident.severity]}</p>
              </div>
            ) : null}
            <div className="max-h-[16rem] space-y-2 overflow-y-auto pr-1 text-xs">
              {incidentClusters.slice(0, 8).map((cluster) => (
                <div key={cluster.id} className="rounded-sm border border-zinc-700/80 bg-zinc-900/60 px-2 py-2">
                  <p className="uppercase tracking-[0.08em] text-zinc-200">
                    <ZoneNavLink zone={cluster.region} className="px-0 py-0 text-zinc-200 hover:text-cyan-200" />
                  </p>
                  <p className="mt-1 text-zinc-400">{cluster.count} events | {classificationLabel[cluster.severity]}</p>
                </div>
              ))}
              {incidentClusters.length === 0 ? <p className="text-zinc-500">No aggregated incidents.</p> : null}
            </div>
          </section>
        </aside>
      </div>
